...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        newPosDistAll = util.Counter()
        #print(gameState.getNumAgents())

        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                newPosDistAll[newPos] += prob * self.beliefs[oldPos]
        
        newPosDistAll.normalize()
        self.beliefs = newPosDistAll
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while particleCount < self.numParticles:
            for p in range(len(self.legalPositions)):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    #countP = [particle for particle in self.particles if particle == p]
            ...
            if allPossible.totalCount() == 0:
                ...
                for particle in range(self.numParticles):
                    self.particles[particle] = util.sample(allPossible)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                newPosDistAll[newPos] += prob

        newPosDistAll.normalize()

        for particle in range(self.numParticles):
            self.particles[particle] = util.sample(newPosDistAll)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        particleCount = 0
        legalPositionsComb = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        #print(legalPositionsPerm)

        while particleCount < self.numParticles:
            for i in range(len(legalPositionsComb)):
                if particleCount < self.numParticles:
                    self.particles.append(legalPositionsComb[i])
                    particleCount += 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        byGhost = [util.Counter() for i in range(self.numGhosts)]
        #print(byGhost)
            ...
            if noisyDistances[i] == None:
                ...
                for p in self.particles:
                    ...
                    if emissionModels[i][trueDistance] > 0:
                        byGhost[i][p[i]] += emissionModels[i][trueDistance]
                if byGhost[i].totalCount() == 0:
                    self.initializeParticles()
                else:
                    for p in range(self.numParticles):
                        self.particles[p] = list(self.particles[p])
                        self.particles[p][i] = util.sample(byGhost[i])
                        self.particles[p] = tuple(self.particles[p])
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for g in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), g, self.ghostAgents[g])
