...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        sortedLivingGhostPosDist = sorted(livingGhostPositionDistributions[0].items(), key=operator.itemgetter(1), reverse=True)
        #print(sortedLivingGhostPosDist)
        closestGhost = sortedLivingGhostPosDist[0][0]
        distance = float("inf")
        bestAction = legal[0]
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            actionDistance = self.distancer.getDistance(closestGhost, successorPosition)
